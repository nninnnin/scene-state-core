...
describe("HistoryManager snapshot jump", () => {
  it("체크포인트를 생성하고 롤백하여 상태 복원 확인", () => {
    ...
    const entityA_transform =
      historyManager.state.components.transform["a"]!;

    // is entity A position changed
    expect(entityA_transform.position).toEqual([
      9, 9, 9,
    ]);
    ...
    const rollbackState = historyManager.state;

    expect(
      Object.keys(rollbackState.entities),
    ).toEqual(["a"]);

    expect(
      rollbackState.components.transform["a"]!
        .position,
    ).toEqual([0, 0, 0]);
  });
  ...
  it("존재하지 않는 체크포인트로 롤백 시도 시 no operation 및 상태 참조 유지", () => {
    ...
    historyManager.jumpToCheckpoint("ghost");

    expect(historyManager.state).toBe(before);
  });
  ...
  it("체크포인트로 롤백 시 히스토리 초기화", () => {
    ...
    const CHECKPOINT_KEY_WITH_X = "withX";
    ...
    const CHECKPOINT_KEY_WITH_XY = "withXY";
    ...
    expect(
      Object.keys(historyManager.state.entities),
    ).toEqual(["a", "x"]);
    ...
    expect(historyManager.stacks).toEqual({
      undoStack: [],
      redoStack: [],
    });
  });
  ...
  it("스냅샷 점프 시 preserve option을 이용하면 history 보존 가능", () => {
    ...
    const CHECKPOINT_KEY_WITH_P = "with P";
