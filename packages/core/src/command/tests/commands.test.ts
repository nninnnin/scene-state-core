...
describe("Command system", () => {
  it("AddEntityCommand adds a new entitiy and undo removes it", () => {
    ...
    const addCmd = new AddEntityCommand("E1", "Cube");

    state = applyCommand(state, addCmd);
    expect(state.entities["E1"]).toEqual({
      name: "Cube",
    });
  ...
  it("RemoveEntityCommand removes an entity and undo restores it with transform", () => {
    ...
    const removeCommand = new RemoveEntityCommand(
      "E1",
    );

    let state = applyCommand(initial, removeCommand);
    ...
    state = undoCommand(state, removeCommand);
  ...
  it("RemoveEntityCommand no-ops if entity does not exist", () => {
    ...
    const removeCmd = new RemoveEntityCommand("E404");

    const next = applyCommand(state, removeCmd);
